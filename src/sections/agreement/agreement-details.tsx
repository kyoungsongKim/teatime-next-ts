...
type Props = {
  agreementDetails?: IAgreementDetailItem;
};

export function AgreementDetails({ agreementDetails }: Props) {
  const [currentStatus, setCurrentStatus] = useState(agreementDetails?);
  ...
  const renderFooter = (
    <Box gap={2} display="flex" alignItems="center" flexWrap="wrap" sx={{ py: 3 }}>
      ...
      <Box flexGrow={{ md: 1 }} sx={{ textAlign: { md: 'right' } }}>
        ...
        <Typography variant="body2">kskim@castis.com</Typography>
      </Box>
    </Box>
  );
